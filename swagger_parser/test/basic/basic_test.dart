...
''';
  ...
  try {
    ...
    if (runResult.exitCode != 0) {
      ...
      throw Exception(
          'Swagger parser failed for ${p.basename(schema.path)} & $jsonFormatter');
    }
    // Check that a folder named api was created

    if (!apiClientFolder.existsSync()) {
      throw Exception(
          'Swagger parser did not create the api folder for ${p.basename(schema.path)} & $jsonFormatter');
    }
    // Run the builder
    ...
    if (buildResult.exitCode != 0) {
      ...
      throw Exception(
          'Build Runner failed for ${p.basename(schema.path)} & $jsonFormatter');
    }
    // Run the analyzer
    ...
    if (analyzeResult.exitCode != 0) {
      ...
      throw Exception(
          'Invalid code generated using ${p.basename(schema.path)} & $jsonFormatter');
    } else {
