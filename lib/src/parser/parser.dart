...
import 'package:swagger_parser/src/parser/parser_exception.dart';
...
class OpenApiJsonParser {
  OpenApiJsonParser(String jsonPath) {
    final jsonFile = jsonInfo(jsonPath);
    if (jsonFile == null) {
      throw ParserException("Can't find json file at $jsonPath");
    }
    final fileContent = jsonFile.readAsStringSync();
    _jsonContent = jsonDecode(fileContent) as Map<String, dynamic>;
  ...
  Iterable<UniversalDataClass> parseDataClasses() {
    ...
    entities.forEach((key, value) {
      ...
      if ((value as Map<String, dynamic>).containsKey(_requiredVar)) {
        requiredParams = (value[_requiredVar] as List<dynamic>)
            .map((e) => e.toString())
            .toList();
      }
      ...
      if (value.containsKey(_propertiesVar)) {
        (value[_propertiesVar] as Map<String, dynamic>).forEach(
          (propertyName, propertyValue) {
            final typeWithImport = _arrayWithDepth(
              propertyValue as Map<String, dynamic>,
              name: propertyName,
              isRequired: requiredParams.contains(propertyName) ||
